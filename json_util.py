...
def dumps(obj,valid, *args, **kwargs):
    ...
    return json.dumps(_json_convert(obj,valid, json_options), *args, **kwargs)
...
def _json_convert(obj,validate, json_options=DEFAULT_JSON_OPTIONS):
    """Recursive helper method that converts BSON types so they can be
    converted into json.
    """
    if hasattr(obj, 'iteritems') or hasattr(obj, 'items'):  # PY3 support
        return SON(((k, _json_convert(v,validate, json_options))
                    for k, v in iteritems(obj)))
    elif hasattr(obj, '__iter__') and not isinstance(obj, (text_type, bytes)):
        return list((_json_convert(v,validate, json_options) for v in obj))
    try:
        if validate:
            return default_valid(obj,json_options)
        else:
...
def default_valid(obj, json_options=DEFAULT_JSON_OPTIONS):
    # We preserve key order when rendering SON, DBRef, etc. as JSON by
    # returning a SON for those types instead of a dict.
    if isinstance(obj, ObjectId):
        # return {"$oid": str(obj)}
